...
return [
    ...
    'custom' => [
        ...
        // ---------------------------------------------------------------
        '/\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9]{2})[0-9]{12})\b/' => $c('Credit / Debit Card Number', ['query', 'body']),
        ...
        '/\b(?:\.bash_history|\.zsh_history|\.DS_Store)\b/i' => $w('Sensitive Artefact Probe', ['query', 'path']),
        ...
        '/\{\{[^}]*(?:config|self|request|session|settings|__class__|__globals__|__subclasses__|__mro__|cycler|joiner|lipsum|_self)[^}]*\}\}/is' => $c('SSTI Object Access', ['query', 'body']),
        ...
        '/#\{[^}]*(?:`|system|exec|open|read|eval|IO\.|File\.)[^}]*\}/i' => $c('Ruby Interpolation Injection', ['query', 'body']),
        ...
        // ---------------------------------------------------------------
        '/(?:%0d%0a|%0a|%0d|\r\n|\r|\n)\s*(?:set-cookie|location|content-type|content-length|refresh)\s*:/i' => $e('CRLF / Header Injection', ['query', 'path']),
